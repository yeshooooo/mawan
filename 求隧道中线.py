...
##y = b + rsin(0)
##圆弧最底部坐标为
arc1Bottom_x = 97315.3599 + 2006.0250 * math.cos(187.65*math.pi/180)
arc1Bottom_y = 16885.2264 + 2006.0250 * math.sin(187.65*math.pi/180)
...
arc1_length = 6.65 * math.pi *2006.0250/180
...
#                起点 角度      1
#                端点 角度     15
arc2Bottom_x = 93311.2628 + 1993.9750 * math.cos(1*math.pi/180)
arc2Bottom_y = 17170.7258 + 1993.9750 * math.sin(1*math.pi/180) ##线段1的终点
#########线段1的长度为

line1_length = math.sqrt((arc1top_x - arc2Bottom_x)**2 + (arc1top_y - arc2Bottom_y)**2)

####
arc2top_x = 93311.2628 + 1993.9750 * math.cos(15*math.pi/180)#
arc2top_y = 17170.7258 + 1993.9750 * math.sin(15*math.pi/180)#此处的xy为线段2的起点
arc2_length = 14 * math.pi *1993.9750/180
...
step = 1.995  ###因为后续误差的原因，经过试验，当步长取2.01时，正好能分成1030环

###每段环数

num_arc1 = int(arc1_length/ step) ###121  116
print(num_arc1)
num_line1 = int(line1_length/step) ###176  178
print(num_line1)
num_arc2 = int(arc2_length/step) ### 244
print(num_arc2)
num_line2 = int(line2_length/step) ###489  492
print(num_line2)
...
###############################################################
####圆弧段 116环，每一环的度数为 188 - i* 7 / 121.0
##最后一环的坐标不取，将误差平均进最后一环，坐标取圆弧段实际的的顶部,在线段部分添加

for i in range(1,116):
    tem_degree = 187.65 - i* 6.65 / 116.0
...
######################################################
###直线1 段，共计178环，但是起点为圆弧段终点，所以最终添加点个数也为176个，为了逻辑上的统一，最后一环的终点，放在下一段圆弧处添加
line1_x_np = np.linspace(arc1top_x,arc2Bottom_x,179)
line1_y_np = np.linspace(arc1top_y,arc2Bottom_y,179)
...
for i in range(1,244):
    ...
    tem_x = 93311.2628 + 1993.9750 * math.cos(tem_degree * math.pi / 180)
    tem_y = 17170.7258 + 1993.9750 * math.sin(tem_degree * math.pi / 180)
    X.append(tem_x)
    Y.append(tem_y)
######最后一段直线  492环,所以一共493个点，包括起点和终点
line2_x_np = np.linspace(arc2top_x,line2_end_x,493)
line2_y_np = np.linspace(arc2top_y,line2_end_y,493)
